...
namespace Jarditech.Views
{
    ...
    public partial class ChoixMachineWindow : Window
    {
        ...
        {
            ChoixVM.SelectedMachine = ChoixVM.Machines.New_machine(ChoixVM.Machin_selecte_name);

            if (ChoixVM.SelectedMachine!=null)
            { 
                ChoixVM.Machines.AddMachin(ChoixVM.SelectedMachine);

                EncodageNouvelleMachineWindow CodeNewMachine = new EncodageNouvelleMachineWindow(ChoixVM.Clients, ChoixVM.SelectedMachine, ChoixVM.Machines);
                CodeNewMachine.Show();
                this.Close();
            }
            else
